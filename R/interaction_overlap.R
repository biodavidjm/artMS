...
interaction_overlap <- function(results, out_file, annotate=T, plot.venn=T){
  ...
  if(!dir.exists(dirname(out_file))) dir.create( dirname(out_file), recursive = T )
  
  x = data.frame(t(combn(unique(results$dataset),2)), stringsAsFactors=F)  # get all combos of data sets
  # remove negatives from this
  x <- x[!unlist(apply(x,1, function(y){ return(any(grepl("MOCK", y))) })),]
  ...
  for(i in 1:dim(x)[1]){
    ...
    i.results <- data.frame(dataset1 = x[i,1], dataset2=x[i,2],
                            pval=pval.dat1_v_dat2$p.value,
                            overlap=m11, dat1_not_dat2=m12, dat2_not_dat1=m21,
                            dataset1.thresh=length(dat1.thresh), dataset2.thresh=length(dat2.thresh),
                            dataset1.size=length(dat1.all), dataset2.size=length(dat2.all)
    )
    # backup results
    ...
    if(m11>0){
      i.intersections = data.frame(dataset1 = x[i,1], dataset2=x[i,2], Protein=intersect(dat1.thresh, dat2.thresh), pval=pval.dat1_v_dat2$p.value, stringsAsFactors = F)
    ...
    if(plot.venn){
      ...
      ## Deprecated
      # tmp = results[which( (results$dataset %in% x[i,]) & (results$thresh==1) ),]
      # venn_out <- gsub(basename(out_file), paste0(x[i,1],"_vs_", x[i,2],".pdf"), out_file)
      # pdf(venn_out)
      # plotVenn(tmp, pval=pval.dat1_v_dat2$p.value)
      # dev.off()
    }
    
    
    # update overlap table from results
    overlap.table <- rbind(overlap.table, data.frame(dataset=paste(x[i,1], x[i,2], sep="-"), 
                                                     overlap=m11, set1only=m12, set2only=m21, 
                                                     p.overlap=m11/sum(m11,m12,m21), p.set1only = m12/sum(m11,m12,m21),
                                                     p.set2only=m21/sum(m11,m12,m21),
                                                     stringsAsFactors = F ) )
  }
  cat('DONE\n')
  ...
  if(sum(overlap.table$overlap>0)>1){
    ...
    if(annotate==1){
      keys <- suppressMessages(select(org.Hs.eg.db, keys=unique(as.character(intersections$Protein)), columns = c("ENTREZID","GENENAME","SYMBOL","UNIPROT"), keytype="UNIPROT"))
...
# This will create and save a venn digram that displays both the number of 
#   members ber group, but also the percentage of the whole group
#' @export
plotVenn2 <- function(m11, m12, m22, pval=NULL){
  ...
  venn.plot = draw.pairwise.venn(area1=m12+m11, area2=m21+m11, cross.area=m11, 
                                 c(x[i,1], x[i,2]), 
...
proportionPlot <- function(x, out_file){
  ...
  # create a plot based on the PROPORTION of overlap. 
  #   Numbers on the plot are actual counts of proteins in that section
  p <- ggplot(x.long, aes(x=dataset, y=percentage)) + 
    geom_bar(stat='identity', aes(fill=variable)) + 
    scale_fill_manual(values=c("red", "purple", "royalblue")) +
    labs(y = "Percentage", x = "Compared Datasets", fill = "Dataset", title="Number of Genes Found") +
    geom_text(data=x.long, aes(x = dataset, y = pos, label = count), size=4) + 
    theme(plot.title = element_text(hjust = 0.5)) + coord_flip()
  
  # Save the plot
  ggsave(filename=gsub(".txt", "_count_all.pdf", out_file), plot=p, width=7, height = max(7, (2*7*length(unique(x.long$dataset))/100) ), limitsize = FALSE)
  p
  
  
  # create a plot based on the PERCENTAGE of overlap. 
  #   Numbers on the plot are actual PERCENTAGE of proteins in that section
  p <- ggplot(x.long, aes(x=dataset, y=percentage)) + 
    geom_bar(stat='identity', aes(fill=variable)) + 
    scale_fill_manual(values=c("red", "purple", "royalblue")) +
    labs(y = "Percentage", x = "Compared Datasets", fill = "Dataset", title="Number of Genes Found") +
    geom_text(data=x.long, aes(x = dataset, y = pos, label = round(percentage,1)), size=4) + 
    theme(plot.title = element_text(hjust = 0.5)) + coord_flip()
  
  # Save the plot
  ggsave(filename=gsub(".txt", "_percentage_all.pdf", out_file), plot=p, width=7, height = max(7, (2*7*length(unique(x.long$dataset))/100) ), limitsize = FALSE)
  p
...
checkDir <- function(output_dir){
  if( !dir.exists(dirname(output_dir)) ) dir.create(output_dir, recursive = T)
  return(output_dir)
}
...
# INPUT: dataframe:
#           Protein = unique identifier for the protein, such as Uniprot Accession
#           dataset = The name of the group/dataset/bait associated with this protein. (interaction)
#           thresh =  Whether the protein is to be considered a confident interactor in this dataset.
#        plot.venn =  Whether or not to plot and save the venn diagrams of the overlaps. May not want to if there are a ton of comparisons.
# head(results)
# out_file = '~/Desktop/overlap_test/overlap.txt'
# 
# 
# fishersBatch(results, out_file, annotate=T, plot.venn=T)
