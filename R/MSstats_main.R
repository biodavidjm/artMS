...
# suppressMessages(library(getopt)) suppressMessages(library(yaml)) suppressMessages(library(biomaRt))

# # set source directory args <- commandArgs(trailingOnly = F) scriptPath <- normalizePath(dirname(sub('^--file=', '',
# args[grep('^--file=', args)])))

## load all external files source('MSstats_functions.R')
...
aggregateData = function(data_w, keys, config, castFun) {
    ...
    aggregate_fun = tryCatch(eval(parse(text = config$aggregation$aggregate_fun)), error = function(e) cat("\tWARNING: argument for aggregate_fun not a valid R function - defaulting to:\t 'max'"), 
        finally = max)
    
    data_l_combined_agg = data.table(aggregate(Intensity ~ RawFileCombined + Proteins + Sequence + Charge + IsotopeLabelType, FUN = sum, 
        data = data_l_combined))
...
normalizeData = function(data_w, config) {
    ...
    } else if (grepl("reference", config$normalization$method) && !is.null(config$normalization$reference) && config$normalization$reference %in% 
        unique(data_w$Proteins)) {
        
        ## CURRENTLY UING MSSTATS METHODS FOR REFERENCE NORMALIZATION
        data_fn = data_w
        
        # cat(sprintf('\tNORMALIZATION\tTO REFERENCE\t%s\n',config$normalization$reference)) ref_files =
        # keys[keys$NormalizationGroup == 'REFERENCE', 'RawFile'] data_l_ref = data_l[data_l$Raw.file %in% ref_files & data_l$Intensity
        # > 0 & is.finite(data_l$Intensity), ] data_l_nonref = data_l[!(data_l$Raw.file %in% ref_files) & data_l$Intensity > 0 &
        # is.finite(data_l$Intensity), ] data_l_ref_n = normalizeToReference(data_l_ref=data_l_ref, ref_protein =
        # config$normalization$reference, output_file = config$files$output) data_fn= rbind(data_l_ref_n, data_l_nonref) data_fn =
        # castMaxQToWide(data_fn)
...
runMSstats = function(dmss, contrasts, config) {
    ...
    if (grepl("before", config$msstats$profilePlots)) {
        mssquant = dataProcess(raw = dmss, normalization = F, betweenRunInterferenceScore = config$msstats$interference, fillIncompleteRows = T, 
            summaryMethod = config$msstats$summaryMethod, censoredInt = config$msstats$censoredInt, cutoffCensored = config$msstats$cutoffCensored, 
            MBimpute = config$msstats$MBimpute, featureSubset = config$msstats$feature_subset)
    ...
    # Normalization if globalStandars is selected, must have a reference protein(s)
    if (!is.null(config$msstats$normalization_reference) & config$msstats$normalization_method == "globalStandards") {
        ...
        # mssquant = dataProcess(dmss, normalization=config$msstats$normalization_method, nameStandards=normalization_refs ,
        # fillIncompleteRows=T)
        mssquant = dataProcess(raw = dmss, normalization = config$msstats$normalization_method, nameStandards = normalization_refs, 
            betweenRunInterferenceScore = config$msstats$interference, fillIncompleteRows = T, summaryMethod = config$msstats$summaryMethod, 
            censoredInt = config$msstats$censoredInt, cutoffCensored = config$msstats$cutoffCensored, MBimpute = config$msstats$MBimpute, 
            featureSubset = config$msstats$feature_subset)
    } else {
        cat(sprintf(">> NORMALIZATION\t%s\n", config$msstats$normalization_method))
        # mssquant = dataProcess(dmss, normalization=config$msstats$normalization_method , fillIncompleteRows = F,
        # betweenRunInterferenceScore = F)
        mssquant = dataProcess(raw = dmss, normalization = config$msstats$normalization_method, betweenRunInterferenceScore = config$msstats$interference, 
            fillIncompleteRows = T, summaryMethod = config$msstats$summaryMethod, censoredInt = config$msstats$censoredInt, cutoffCensored = config$msstats$cutoffCensored, 
            MBimpute = config$msstats$MBimpute, featureSubset = config$msstats$feature_subset)
    }
    ...
    write.table(mssquant$ProcessedData, file = gsub(".txt", "-mss-normalized.txt", config$files$output), eol = "\n", sep = "\t", 
        quote = F, row.names = F, col.names = T)
    # results = groupComparison(data = mssquant, contrast.matrix = contrasts, labeled = as.logical(config$msstats$labeled),
    # scopeOfBioReplication = config$msstats$scopeOfBioReplication, scopeOfTechReplication = config$msstats$scopeOfTechReplication,
    # interference = as.logical(config$msstats$interference), equalFeatureVar = as.logical(config$msstats$equalFeatureVar),
...
Extras.annotate = function(results, output_file = "MSstat_results.txt", uniprot_ac_col = "Protein", group_sep = ";", uniprot_dir = "~/github/kroganlab/source/db/", 
    species = "HUMAN") {
    cat(">> ANNOTATING\n")
...
writeExtras = function(results, config) {
    ...
    if (config$output_extras$annotate & is.null(config$filters$modifications)) {
        results_ann <- Extras.annotate(results, output_file = config$files$output, uniprot_ac_col = "Protein", group_sep = ";", 
            uniprot_dir = config$output_extras$annotation_dir, species = config$output_extras$species)
    } else {
        if (!is.null(config$filters$modifications)) 
            cat("\tSITES NEED TO BE MAPPED BACK TO PROTEINS BEFORE ANNOTATING.\n")
        results_ann = results
    ...
    if (config$output_extras$heatmap) {
        ...
        heat_data_w = plotHeat(mss_F = sign_hits, out_file = gsub(".txt", "-sign.pdf", config$files$output), names = heat_labels, 
            cluster_cols = config$output_extras$heatmap_cluster_cols, display = config$output_extras$heatmap_display)
    }
    
    if (config$output_extras$volcano) {
        cat(">>   PLOTTING VOLCANO PLOT\n")
        file_name = gsub(".txt", "-volcano.pdf", config$files$output)
        volcanoPlot(results_ann[grep(selected_labels, results_ann$Label), ], lfc_upper, lfc_lower, FDR = config$output_extras$FDR, 
            file_name = file_name)
    }
}
...
# @param input_file The filepath to the MaxQuant searched results file (txt tab delimited file). This can either be in either
# long or wide format.  @param keys_file The filepath to the keys file used in the MSStats analysis.  @param ref_proteome_file
# The filepath to the reference proteom (txt tab delimited file).
...
MQ.msstats <- function(opt) {
    ...
    config = tryCatch(yaml.load_file(opt$config), error = function(e) {
        cat(opt$config)
        break
    })
    ...
    if (config$data$enabled) {
        cat(">> LOADING DATA\n")
        # data = fread(config$files$data, stringsAsFactors=F, integer64 = 'double') # Found more bugs in fread. Not worth the
        # compormise in data integrity just to save time reading in data
        ...
        if (config$files$sequence_type == "modified") 
            castFun = castMaxQToWidePTM else castFun = castMaxQToWide
        ...
        ## NORMALIZATION -- we normalize with MSstats now if(config$normalization$enabled) data_fn = normalizeData(data_w, config) else
        ## data_fn=data_w
    ...
    if (config$output_extras$enabled) {
        if (!config$msstats$enabled) 
            results = read.delim(config$output_extras$msstats_output, stringsAsFactors = F)
...
# spec = matrix(c( 'verbose', 'v', 2, 'integer', '', 'help' , 'h', 0, 'logical', 'available arguments (this screen)', 'config'
# , 'c', 1, 'character', 'configuration file in YAML format'), byrow=TRUE, ncol=5) opt = getopt(spec = spec, opt =
# commandArgs(TRUE), command = get_Rscript_filename(), usage = FALSE, debug = FALSE) # if help was asked for print a friendly
# message # and exit with a non-zero error code if ( !is.null(opt$help) ) { cat(getopt(spec, usage=TRUE)); q(status=1); }

## TEST WORKS WITH LATEST CODE opt = c(opt, config='tests/LabelFree-ub/LabelFree-ub-test.yaml')

# if(!exists('DEBUG')){ cat('>> RUN MODE\n') system.time(main(opt)) }else{ cat('>> DEBUG MODE\n') } # rm(list = ls(all =
# TRUE))
