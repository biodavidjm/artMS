...
# Input an evidence file from MaxQuant and a file containing a list of proteins of interest.
# Output a file containing averages of Intensity, Retention Time, Caliberated Retention Time

#' @title Summarize average intensity and retention time per protein
#' @description This will summarize the evidence file and report back the average intensity, average retention time, and the average caliberated retention time. If a list of proteins is provided, then only those proteins will be summarized and returned.
#' @param evidence_file The filepath to the MaxQuant searched data (evidence) file (txt tab delimited file).
#' @param protein_file The filepath to a file or vector conatining a list of proteins of interest.
#' @param output_file The filepath to where you want the results saved. If no file path is given, then the results will be returned as an R object.
...
avg_intensity_RT <- function(evidence_file, protein_file=NULL, output_file=FALSE){
  ...
  dat.avg <- aggregate(data=dat[,c("Proteins","Gene_names","Modified_sequence","Charge","Intensity")], Intensity~., mean, na.rm=T)
  # Compute the average Retention Time
  dat.ret <- aggregate(data=dat[,c("Proteins","Gene_names","Modified_sequence","Charge","Retention_time")], Retention_time~., mean, na.rm=T)
  # Compute the average Calibrated retention time
  dat.cal <- aggregate(data=dat[,c("Proteins","Gene_names","Modified_sequence","Charge","Calibrated_retention_time")], Calibrated_retention_time~., mean, na.rm=T)
...
#   'file'  , 'f', 1, "character", "Path to the evidence file to summarize.",
#   'proteins'  , 'p', 1, "character", "Path to file containing a list of proteins to be searched for."),
#   byrow=T, ncol=5)
#
#
# opt = getopt(spec = spec, opt = commandArgs(TRUE), command = get_Rscript_filename(), usage = FALSE, debug = FALSE)
#
# # Handle the case where help is used
# if ( !is.null(opt$help) ) {
#   cat(getopt(spec, usage=TRUE));
#   q(status=1);
# }
#
# file_summary(opt$file, opt$proteins)
