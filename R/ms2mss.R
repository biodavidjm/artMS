...
#' mq2mss()
#' @export
mq2mss <- function(evidence_file, keys_file, protein_groups="ignore", contaminants=TRUE, modification=NULL, plot.heatmap=FALSE, output_file=NULL, aggregate_intensities=FALSE, aggregate_fun=NULL){
  ...
  if(plot.heatmap){
    ...
    if(is.null(output_file)) stop('No output file designated for heatmap! Please enter a file path for the heatmap to be saved.\n')
  ...
  ## AGGREGATION
  # Aggregate Intensitis based on unique RawFileCombined + Proteins + Sequence + Charge + IsotopeLabelType combinations
