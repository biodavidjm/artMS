...
.select_ref <- function(keys_file, dat_file) {
    ...
    x.clust$prot_names <- paste(x.clust$Gene.names, x.clust$Proteins, sep = "-")
    
    
    # add in bait names
    x.clust <- merge(x.clust, keys[, c("RawFile", "Condition")], by.x = "variable", by.y = "RawFile")
    x.clust$sample_name <- paste(x.clust$variable, x.clust$Condition, sep = "-")
    ...
    serv <- shiny::shinyServer(function(input, output) {
        ...
        output$text1 <- shiny::renderText({
            idx <- intersect(grep(paste(input$check_proteins, collapse = "|"), x.clust$prot_names), which(x.clust$cluster %in% 
        ...
        output$plot1 <- plotly::renderPlotly({
            idx <- intersect(grep(paste(input$check_proteins, collapse = "|"), x.clust$prot_names), which(x.clust$cluster %in% 
                input$check_clusters))
            p <- ggplot(data = x.clust[idx, ], aes(x = sample_name, y = value, colour = prot_names)) + geom_line(aes(group = prot_names)) + 
        ...
        output$plot_clusts <- plotly::renderPlotly({
            ...
            idx <- intersect(grep(paste(input$check_proteins, collapse = "|"), x.clust$prot_names), which(x.clust$cluster %in% 
                input$check_clusters))
            p <- ggplot(data = x.clust[idx, ], aes(x = sample_name, y = value, colour = cluster)) + geom_line(aes(group = prot_names)) + 
                theme(axis.text.x = element_text(angle = 45, hjust = 1))
            plotly::ggplotly(p)
        })
    })
    ...
    ui <- shiny::shinyUI(shiny::basicPage(shiny::titlePanel("Reference Protein Selection"), shiny::sidebarLayout(shiny::div(style = "width: 1000px", 
        shiny::sidebarPanel(shiny::checkboxGroupInput("check_proteins", "Proteins displayed:", sort(unique(x.clust$prot_names)), 
            selected = unique(x.clust$prot_names)))), shiny::div(align = "left", shiny::mainPanel(shiny::tabsetPanel(shiny::tabPanel("Main", 
        plotly::plotlyOutput("plot1", height = "800px")), shiny::tabPanel("Clusters", shiny::checkboxGroupInput("check_clusters", 
        "Proteins displayed:", sort(unique(x.clust$cluster)), selected = unique(x.clust$cluster), inline = TRUE), plotly::plotlyOutput("plot_clusts", 
