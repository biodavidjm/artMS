...
#' @param species (char) Specie name. If several, used a `dash` symbol 
#' to separate them
...
artms_evidenceToMISTformat <-
  function(input_file,
    ...
    if (quant_variable == "msint") {
      ...
      stop("\n << ",
           quant_variable,
           " >> NOT ALLOWED. ONLY msint OR msspc ALLOWED\n")
    }
    ...
    if (is.null(uniprot_dir)) {
      uniprot_dir <- '~/Box Sync/db/mist/'
      cat("\t---uniprot_dir NOT SPECIFIED. USING DEFAULT:  ",
          uniprot_dir,
          "\n")
    }
    
    data <-
      data.table(read.delim(input_file, stringsAsFactors = FALSE))
    keys <-
      data.table(read.delim(keys_file, stringsAsFactors = FALSE))
    
    cat('\tCHECKING DATA AND KEYS COLUMN NAMES\n')
    tryCatch(
      setnames(data, 'Raw file', 'RawFile'),
      error = function(e)
        cat('\t---Raw file in evidence not found: trying Raw.file instead\n')
    )
    tryCatch(
      setnames(data, 'Raw.file', 'RawFile'),
      error = function(e)
    ...
    tryCatch(
      ...
      error = function(e)
        cat('\t---Raw file in keys not found: trying Raw.file instead\n')
    )
    tryCatch(
      setnames(keys, 'Raw.file', 'RawFile'),
      error = function(e)
    ...
    if (quant_variable == "msspc") {
      tryCatch(
        setnames(data, 'MS/MS Count', 'ms_spectral_counts'),
        error = function(e)
          cat(
            '\t---MS/MS Count column not found in evidence file: trying MS.MS.Count instead\n'
          )
      )
      tryCatch(
        setnames(data, 'MS.MS.Count', 'ms_spectral_counts'),
      ...
      tryCatch(
        setnames(data, 'Intensity', 'ms_intensity'),
        error = function(e)
          stop('\t--- INTENSITY NOT FOUND IN THE evidence FILE!!\n\n')
      )
    } else{
      stop("\n << ",
           quant_variable,
    ...
    if (quant_variable == "msspc") {
      data_sel <-
        data[, c('Proteins',
                 ...
                 'Run',
                 'RawFile',
                 'ms_spectral_counts'), with = FALSE]
      data_sel <-
        aggregate(
          ms_spectral_counts ~ Proteins+Condition+BioReplicate+Run+RawFile,
          data = data_sel,
          FUN = sum
        )
    } else if (quant_variable == "msint") {
      data_sel <-
        data[, c('Proteins',
                 ...
                 'Run',
                 'RawFile',
                 'ms_intensity'), with = FALSE]
      data_sel <-
        aggregate(
          ms_intensity ~ Proteins + Condition + BioReplicate + Run + RawFile,
          data = data_sel,
          FUN = sum
        )
    } else{
    ...
    if (length(grep(";", data_sel$Proteins)) > 0)
      data_sel = data_sel[-grep(";", data_sel$Proteins), ]
    ...
    # (this column is generated by Prospector and it is used to eleminate 
    # peptides. In this case, does not apply but it has to be there)
    ...
    if (quant_variable == "msspc") {
      ...
      data_sel <-
        data_sel[, c("RawFile", 'Proteins', 'ms_unique_pep', 'ms_intensity')]
      # RENAMING!
      names(data_sel) <-
        c('id', 'ms_uniprot_ac', 'ms_unique_pep', 'ms_intensity')
    ...
    for (org in species_split) {
      ...
      tmp <-
        read.delim(
          sprintf(
            "%s/uniprot_protein_descriptions_%s.txt",
            uniprot_dir,
            org
          ),
    ...
    results_annotated = merge(data_sel,
                              Uniprot,
                              all.x = TRUE,
                              by.x = 'ms_uniprot_ac',
                              by.y = 'Entry')
    ...
    # For that, we will calculate the mass of the whole protein just taking the 
    # average molecular weight of an amino acid: 110Da
