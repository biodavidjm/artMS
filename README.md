...
artMS is an R package that provides a set of tools for the analysis and integration of large-scale mass-spectrometry-based proteomics datasets obtained using the popular proteomics software package 
[MaxQuant](http://www.biochem.mpg.de/5111795/maxquant).

artMS facilitates and simplifies the analysis on R by creating a configuration file where multiple options can be selected, including:

- Multiple quality control (QC) functions.
- Relative quantification using MSstats.
- Post-MSstats processing and integration.
- A set of miscellaneous functions to generate input files for other popular tools, including:
    - APMS: MIST and CompPASS
    - APMS: SAINTq
    - APMS: SAINT express
    - Photon
    - Phosfate
...
- The `evidence.txt` file, which is obtained with MaxQuant.
- A `keys.txt` file describing the whole experimental designed (generated by the user, templates available)
- A `contrast.txt` file with the comparisons between conditions that the user wished to be quantified (generated by the user, templates available).

Finally, among the most relevant feature of `artMS`, a configuration file (in `yaml` format) is used to specify all the set of available (generated by the user, templates available).
