...
Finally, among the most relevant feature of `artMS` takes as a input for 
quantifications a configuration file (in `yaml` format) with the details 
of the quantifications and other outputs.
...
But before you start, make sure that you are running an `R version >= 3.5` 
otherwise nothing will work (bioconductor requirements). 
...
If the outcome is >= 3.5, congratulations! you can move forward

*If it is not, then you need to [install the latest version of R in your system](https://www.r-project.org/).*

After updating to the latest R version, open RStudio, and type again 
`getRversion()` to make sure it worked.

Once you have the latest (`R >= 3.5`) version running in your system, 
follow these steps:

- Install these packages directly from bioconductor. Why? The installation of 
these packages returns warnings that do not affect the functionality of `artMS` 
but are affecting the installation). `artMS` is using them for annotation purposes.
...
- Check that it is up an running
...
control functions on the sample "evidence" (`artms_data_ph_evidence`) and 
"keys" (`artms_data_ph_keys`) files included in `artMS`
...
Example: (see `artms_data_ph_keys`)
...
The configuration file in `yaml` format contains the configuration details for
the quantification performs by `artMS` using `MSstats`. 
...
`artMS` currently covers the quantification of protein abundance, 
