...
```{r}
artms_main(yaml_config_file = '/path/to/config/file/artms_ab_config.yaml')
```
...
Comprehensive analysis of the quantification obtained running `artms_main()`. It includes:
...
It takes as input two files generated from the previous step (`artms_main()`)
...
1. Set as the working directory the folder with the results obtained from `artms_main()`.
