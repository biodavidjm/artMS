...
As an example, we will use a REDUCED version of a phosphorylation dataset, 
which is available in `artMS`.

- Evidence file: `artms_data_ph_evidence`
- Keys file: `artms_data_ph_keys`

The full data set (large size) can be found at these urls:
...
Then run `artms_evidenceQCbasic()` and specify the type of phosphoproteomics 
experiment (argument `prot_exp = 'PH'`) like this:
...
Check `artms_evidenceQCbasic()` to find out more about this function. 
...
Taking as input the evidence file location, it will summarize and report back the average intensity, average retention time, and the average caliberated retention time for each protein. If a list of proteins is provided, then only those proteins will be summarized and returned. Check `?artms_avg_intensity_RT()` to find out more options.

it generates reproducibility plots, as specificied by the user. It requires an
additional file with the following information:
